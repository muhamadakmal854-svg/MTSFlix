...
object DeviceCodeManager {
    ...
    /**
     * Check if verified locally without forcing the 24-hour online re-check.
     * Used for offline fallback to avoid blocking users when they have no network.
     */
    fun isVerifiedLocallyOffline(context: Context): Boolean {
        val prefs = prefs(context)
        if (!prefs.getBoolean(KEY_VERIFIED, false)) return false
